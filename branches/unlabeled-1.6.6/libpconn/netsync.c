 ...
 *
 * $Id: netsync.c,v 1.6.6.1 2001-07-28 18:05:54 arensb Exp $
 */
		    ...
		    int no_header)	/* m50x starts without a header! */
{
	...
	if (!no_header) {
		/* Read packet header */
	  	err = pconn->io_read(pconn->fd, hdr_buf, NETSYNC_HDR_LEN);
		if (err < 0)
		{
	...
	while (want > got)
	{
		err = pconn->io_read(pconn->fd, pconn->net.inbuf+got, want-got);
		if (err < 0)
		{

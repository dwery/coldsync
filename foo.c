...
dlpcmd_debug = 100;
	...
	err = DlpReadFeature(fd, 'netl', 0, &featvalue);
	if (err < 0)
	{
...
/*  dlp_debug = 0; */
	...
	for (i = 0; i < numrecs; i++)
	{
		fprintf(stderr, "\tID %d: %ld (0x%04lx)\n",
			i, recids[i], recids[i]);
	}
...
memcpy(buf, readret.data, readret.size);
	...
	for (i = 0; ; i++)
	{
		...
		fprintf(stderr, "\tdata:\n");
		debug_dump("\t| ", buf, resource.size);
	}

	/* Read a resource by type */
	err = DlpReadResourceByType(fd, dbh, 'ownr', 1, 0, 0xffff,
				    &resource, buf);
	if (err < 0)
	{
		fprintf(stderr, "Error during DlpReadResourceByIndex: (%d) %s\n",
	...
	buf[0] = '\0';
	buf[1] = '\1';
	sprintf(buf+2, "Ce pilote appartient a:\nHello, world! (%ld)\n",
		time(NULL));
	err = DlpWriteResource(fd, dbh, 'ownr', 1, strlen(buf+2)+3, buf);
	if (err < 0)
	{
